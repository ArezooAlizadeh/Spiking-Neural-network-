...
if neuronparams == True: # testing phase
...
#        start_scope()
        ...
        sigma = array([ 0.5 ])  
#        sigmas=array([0.0, 0.05, 0.1 , 0.15, 0.2 , 0.25])
        for alpha in np.array([ 0.54772727,  0.45227273,  0.33181818,  0.16590909,  0. , -0.14545454, -0.21590909, -0.38636364, -0.53181818]):
...
#            jj=ii+14
            tamp=15
#        for ii in arange(1,29):
            
#            F0=[2.4,2.5,2.6,2.7,2.8,2.9][index]*1e-12
            F0=3*1e-12
            W1=10
            W2=4
       
#            names= 'inpSpi='+str(sigma)+',tamp='+str(tamp)
#            names= 'inpSpi_'+str(tamp)+'tamp'

#            [indice,times,inpSpi]  = pickle.load(open(names,'rb'))  


#            sigma=Hello[jj][0]
#            m=Hello[jj][1]
#             
#            names= 'inpSpi_sigma='+str(np.round(sigma,2))+'_F0='+str(m)+'_winh=0.05.pickle'
#            [indice,times,inpSpi]  = pickle.load(open(names,'rb'))  
            
#            winh=winhs[int(tamp/5)]*1e-3
            duration=550*ms
            tt = linspace(0*second, duration, int(duration/defaultclock.dt))
            
#            name='InpSpi'+str(max_cur[ii])    
#            [times,indice]= pickle.load(open(name,'rb'))
            
#            inputNeuron = SpikeGeneratorGroup(N,indice,times)       
        ...
        #   current = FEFcurrent(beta, F0, distance2, t, Noise=False)
...
#            name = 'getConnections.pickle'
            
#            [fit_fn] = pickle.load(open(name,'rb'))
            T1=55
            tauws = (1+alpha)* np.linspace(T1,T1/60,N)
            weights = np.linspace(W1,W2, N)           
#            weights = np.ones(len(tauws))*5
#            sn = fit_fn(tauws)
#            weights = tauws*1e-1
#            weights = fit_fn(tauws)
            #tauws = fit_fn(weights)
        #    plot(weights, tauws, '.')
...
#                    conSCSCinh.weight_inh[iii,:] = inhw[N-1-iii:2*N-1-iii:]*wamp ## 2017 paper
...
#                    conSCSCexc.weight_exc[iii,:] = excw[N-1-iii:2*N-1-iii:]*wamp
    ...
    #    
            #figure()
            #imshow(conSCSCinh.W.todense(), interpolation='nearest')    
            #figure()
            #imshow(conSCSCexc.W.todense(), interpolation='nearest')  
        #    current = FEFcurrent(beta, 0, distance2, t, Noise=False)
        #    current = FEFcurrent(beta, F0, distance2, t, Noise=False)
        ...
        #    delay=28*ms
            ...
            SCSpikes = SpikeMonitor(SCNeuron)
...
#                cV = StateMonitor(inputNeuron, 'vm', record=[centreid])
#                cw = StateMonitor(inputNeuron, 'w', record=[centreid])
        ...
        #    reinit_default_clock()
            ...
            #figure()
            #a = conSCSCinh.W.todense()      
            #plot(a[centreid, :])
            #a = conSCSCexc.W.todense()
            #plot(a[centreid, :])
            #savefig('lateral.png')
            #close('all')
        ...
        #    if detail:
#                figure()
#                plot(cV_SC.vm[centreid])
#                figure()
#                plot(cw[centreid])
        #        ss = conscheme+'-centneuronid'+str(centreid)+'.pickle'
        #        with open(ss, 'wb') as f:
        #            pickle.dump([cV[centreid[0]], cw[centreid[0]], cge[centreid[0]], cgi[centreid[0]], outSpi[centreid[0]]], f)
#            names= 'saccadesim_'+'sigma='+str(np.round(mag,2))+'_F0='+str(F0)+'_winh='+ str(winh)+'.pickle'
            names='saccadesim,'+'Wfs='+str(W1)+'-'+str(W2)+',tamp='+str(tamp)+',tauw='+str((1+alpha)*T1)+'-'+str((1+alpha)*(T1/60))+',alpha='+str((1+alpha))
#            names='saccadesim'+',tamp='+str(tamp)
#            pickle.dump([i_FEF[:],t_FEF[:],inpSpi,outSpi,centreid],open(names,'wb'))
...
def grid(beta, F0, wmin, wmax, wstep, tauwmin, tauwmax, tauwstep):
    ...
    inputNeuron = NeuronGroup(1, model=eqs1, threshold='vm>Vcut', reset=reset1, method='euler')
...
#inp, out, cent = simulate(1.5, 0.5, 10e-3, 15e-3)
...
#    targetamp=array([15])
    ...
    if dump:
#        name = conscheme+'-networksi'+str(sigmainh)+'se'+str(sigmaexc)+'winh'+str(winh)+'wexc'+str(wexc)+'3to65mf.pickle'
#        with open(name, 'wb') as f:
